...
local json = (loadfile "/home/bot0/inline/data/JSON.lua")()
local token = '357948346:AAGj0rqUDDox6M_89GfjEbgdiN374dzhrzI' --token
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 301772169 or q.from.id == 333677042 then
...
local hash = SUDO..'settings:'..chat..':'..value
 if redis:get(hash) then
    ...
    return false
         ...
         result = '💞ارسال خوشامد گویی فعال شد💞'
								q.data = 'next_page:'..chat
          else
          redis:set(SUDO..'status:welcome:'..chat,'disable')
          result = '💞ارسال خوشامد گویی غیرفعال شد💞'
								q.data = 'next_page:'..chat
									end
								else
								q.data = 'next_page:'..chat
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`قیمت طرح های مربوط به این ربات:`\n`ماهانه(30 الی 31 روز کامل)` >  *7000*\n`سالانه(365 روز کامل)` > *70000*\n`شش ماهه(180 روز)` > *40000*\n`تمامی قیمت ها به` تومان `میباشد.`',keyboard)
            end
			------------@@@@@@@@@@@@@@@@@@@@@@@@@@------------------
