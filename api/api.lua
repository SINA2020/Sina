...
local json = (loadfile "/home/nic8/inline/data/JSON.lua")()
local token = '222894229:AAGflvR33MxPrsjz9ezPTdgSYFeDjCbEx58' --token
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 397293611 or q.from.id == 333677042 then
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`ا بخش راهنما در اختیار صاحبان گروه قرار داده میشودد.`\n`ا خوش آمدید کنید:`',keyboard)
            end
							------------------------------------------------------------------------
...
local function getsettings(value)
       ...
       if ex == -1 then
        ...
        if hash then
            if redis:hget("flooding:settings:"..chat, "flood") == 'kick' then
         return 'اخراج(کاربر)'
              elseif redihgs:hget("flooding:settings:"..chat, "flood") == 'ban' then
              return 'مسدود سازی(کاربر)'
              elseif redis:hget("flooding:settings:"..chat, "flood") == 'mute' then
              return 'سکوت(کاربر)'
              end
